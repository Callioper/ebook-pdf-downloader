...
> **对比本项目**：Agent 版本是 AI 原生设计——由 Agent 按 SKILL.md 指令文件自行编排工具调用和错误恢复，不需要手动操作 Web UI、配置数据库路径、安装 OCR 引擎。本项目是完整的 Web 应用 + REST API，适合需要可视化界面和精细控制的场景。两者可以配合使用：本项目提供 API 后端，Agent 版本通过 API 调用本项目实现自动化。
