...
如果你更喜欢用自然语言操作，试试 **[Agent Ebook Downloader](https://github.com/Callioper/agent-ebook-downloader)** — 无需手动配置，直接对 AI 说"帮我下载《三体》的 PDF 并 OCR"即可自动完成检索、下载、OCR 全流程。

> **比本项目更易上手**：不需要搭建环境、配置数据库路径、手动安装 OCR 引擎。Agent 版本使用 AI 代理自动处理所有步骤，适合非技术用户或快速试用。
